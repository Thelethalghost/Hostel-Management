...
def add_new_data():
        ask = input("Enter the name of the hostel in which you want to add the student in: ")
...
def search():
    ...
    try:
        ...
        def minisearch(table_name,val):
            ...
            header = ('Adminno.','Name','Age','Class','Gender','HostelID')
            print(header)
            for y in myc:
             print(y)
        minisearch(hname,num)
    except Exception:
        print('The adminno. entered is not present')

def  display():
    head = ('HostelID','Hostel Name')
    print(head)
